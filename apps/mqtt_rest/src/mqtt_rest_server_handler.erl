-module(mqtt_rest_server_handler).
-moduledoc """
Exposes the following operation IDs:

- `GET` to `/server/config`, OperationId: `getConfig`:
...
content_types_accepted(Req, #state{operation_id = 'getConfig'} = State) ->
    {[], Req, State};
...
handle_type_accepted(Req, #state{operation_id = OperationID,
    ...
    {Res, Req1, Context1} = Handler(server, OperationID, Req, Context),
