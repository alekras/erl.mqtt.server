...
validate_response('GetAllStatuses', 200, Body, ValidatorState) ->
	validate_response_body('list', 'ConnectionStatus', Body, ValidatorState);
...
validate(Rule = {type, 'integer'}, Name, Value, _ValidatorState) ->
	try
		{ok, mqtt_rest_utils:to_int(Value)}
	catch
		error:badarg ->
...
validate(Rule = {type, 'float'}, Name, Value, _ValidatorState) ->
	try
		{ok, mqtt_rest_utils:to_float(Value)}
	catch
		error:badarg ->
			validation_error(Rule, Name)
	end;
...
validate(Rule = schema, Name, Value, ValidatorState) ->
	Definition =  list_to_binary("#/components/schemas/" ++ mqtt_rest_utils:to_list(Name)),
	try
...
get_value(qs_val, Name, Req) ->
	...
	Value = mqtt_rest_utils:get_opt(mqtt_rest_utils:to_qs(Name), QS),
	{Value, Req};
get_value(header, Name, Req) ->
	Headers = cowboy_req:headers(Req),
	Value =  maps:get(mqtt_rest_utils:to_header(Name), Headers, undefined),
	{Value, Req};
get_value(binding, Name, Req) ->
	Value = cowboy_req:binding(mqtt_rest_utils:to_binding(Name), Req),
	{Value, Req}.
...
binary_to_lower(V) when is_binary(V) ->
	list_to_binary(string:to_lower(mqtt_rest_utils:to_list(V))).
